...
def _online_user_ids(db: Session) -> Set[int]:
    """만료되지 않은 세션을 하나라도 가진(=로그인 상태인) user_id 집합.

    실시간 접속 여부를 알 방법(하트비트/웹소켓)은 없으므로 "유효한 로그인 세션
    보유"를 대리 지표로 쓴다 - 로그아웃하지 않고 창을 닫아도 세션 만료(7일) 전까지는
    온라인으로 표시된다는 한계가 있음.
    """
    rows = (
        db.query(models.Session.user_id)
...
def list_users(
    ...
    online_ids = _online_user_ids(db)
    if online is not None:
        users = [u for u in users if (u.id in online_ids) == online]
...
def get_user_detail(db: Session, user_id: int) -> Optional[schemas.AdminUserDetailOut]:
    ...
    return schemas.AdminUserDetailOut(
        ...
        is_online=active_session_count > 0,
